...
{
    public class BuildFactory : IBuildFactory, IDisposable, IResetable
    {
        private readonly IObjectResolver _resolver;
        private readonly Dictionary<AssetId, IObjectPool<Build>> _pools = new();
        private readonly HashSet<Build> _activeObjects = new();
        ...
        public async UniTask PrewarmPoolAsync(BuildData data, CancellationToken cancellation)
        {
            var prewarmedObjects = new List<Build>(_defaultPoolSize);
        ...
        public void Reset()
        {
            foreach (Build build in new HashSet<Build>(_activeObjects))
            {
                build.Remove();
            }
        }
